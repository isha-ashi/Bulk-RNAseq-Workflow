trimmomatic <- function(dataPath, adapterUsed="TruSeq3-PE.fa", isPairedEnd=TRUE, toExtractFastaFile=NULL, userName="Isha",
                        sbatchTemplateFile="/ix/cigcore/utils/code/trimmomatic_template.sbatch",
                        ...
                        threads=63, doWrite=TRUE, transferPrefix=NULL, minlen=NULL, hours=20){
  library(stringr)
  library(stringi)
  ...
  logPath <- file.path(codePath, "log/trimmomatic")
  ...
  inputFiles <- paste(paste0("$DATAPATH/${FASTQNAMES[${SLURM_ARRAY_TASK_ID}]}", 
                       filEndpattern), collapse = " ")
  outFiles <- paste0("$TRIMOUTPATH/${FASTQNAMES[${SLURM_ARRAY_TASK_ID}]}_trimmomatic", filEndpattern) 
  interimFiles <- paste0("$INTERIMPATH/${FASTQNAMES[${SLURM_ARRAY_TASK_ID}]}", filEndpattern)
  ...
  script  <- gsub(x=script, pattern = "threadsHolder", replace = threads)
  script  <- gsub(x=script, pattern = "fastaFileHolder", replace = adapterUsed)
  script  <- gsub(x=script, pattern = "inputFileHolder", replace = inputFiles)
  if(isPairedEnd){
    script <- gsub(x=script, pattern = "outFileHolder", replace = paste(outFiles[1], interimFiles[1], outFiles[2], interimFiles[2]))
  } else{
    script  <- gsub(x=script, pattern = "outFileHolder", replace = paste(outFiles[1], interimFiles[1]))
  }
  
  sbatchFileName <- paste0(splitDataPath[length(splitDataPath)], "_trimmomatic.sbatch")
  ...
  cleaning <- result
  save(cleaning,file=file.path(logPath,"trimming.RData"))
  return(cleaning)
}